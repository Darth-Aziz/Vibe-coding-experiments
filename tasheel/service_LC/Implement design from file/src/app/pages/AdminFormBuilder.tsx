...
import { 
  ...
  GripHorizontal, FileText, Check, AlertCircle, Copy, Search, ArrowRight,
  Monitor, Box, Briefcase, Workflow, CheckCircle2, Server
} from 'lucide-react';
...
export function AdminFormBuilder() {
  ...
  const [draftSavedAt, setDraftSavedAt] = useState<Date | null>(new Date());
  
  // Step 1: Details
  const [serviceInfo, setServiceInfo] = useState({
    name: 'Hardware Refresh (Laptop)',
    category: 'IT',
    description: 'Please fill out this form to request a replacement for your existing company laptop. Approvals are required from your manager and the IT department before hardware is provisioned.',
    sla: '4 hours',
    icon: 'monitor',
    visibility: 'internal'
  });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Enter to proceed (if not in a textarea)
  ...
  const handleNextStep = () => {
    if (currentStep === 1 && !serviceInfo.name.trim()) {
      toast.error('Service Name Required', { description: 'Please provide a name before continuing.' });
      return;
    }
  ...
  // Views for steps
  const renderStep1 = () => (
    <div className="flex-1 overflow-y-auto bg-muted/5 flex flex-col items-center py-12 px-6">
      <div className="max-w-2xl w-full space-y-8">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Service Details</h2>
          <p className="text-muted-foreground text-sm mt-1">Configure the basic information and visibility for this service.</p>
        </div>

        <Card className="p-6 space-y-6 shadow-sm border-border/50 bg-card">
          <div className="space-y-2">
            <Label className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">Service Name</Label>
            <Input 
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select value={serviceInfo.category} onValueChange={v => setServiceInfo({...serviceInfo, category: v})}>
            ...
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">SLA (Target Response)</Label>
              <Select value={serviceInfo.sla} onValueChange={v => setServiceInfo({...serviceInfo, sla: v})}>
                <SelectTrigger className="bg-background h-10">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-3">
            <Label className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">Service Icon</Label>
            <div className="flex gap-3">
              {['monitor', 'briefcase', 'box', 'server'].map(icon => {
                const isSelected = serviceInfo.icon === icon;
                const IconComponent = icon === 'monitor' ? Monitor : icon === 'briefcase' ? Briefcase : icon === 'box' ? Box : Server;
                return (
                  <button
                    key={icon}
                    onClick={() => setServiceInfo({...serviceInfo, icon})}
                    className={`w-12 h-12 rounded-lg border flex items-center justify-center transition-all ${
                      isSelected ? 'border-primary bg-primary/5 text-primary ring-1 ring-primary/20' : 'border-border/60 bg-background text-muted-foreground hover:bg-muted'
                    }`}
                  >
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border border-border/60 bg-muted/20">
            <div className="space-y-0.5">
              <Label className="text-sm font-medium text-foreground cursor-pointer">Internal Only</Label>
              <p className="text-xs text-muted-foreground">Only logged-in employees can request this service.</p>
            </div>
            <Switch 
              checked={serviceInfo.visibility === 'internal'}
              onCheckedChange={(c) => setServiceInfo({...serviceInfo, visibility: c ? 'internal' : 'public'})}
            />
          </div>
        </Card>
      </div>
        ...
        <div className="max-w-[760px] w-full py-12 px-8 flex flex-col items-center">
          ...
          >
            ...
            <div className="px-10 pt-12 pb-8 border-b border-border/20 group relative">
              <h2 className="text-3xl font-semibold tracking-tight mb-3 text-foreground">{serviceInfo.name || 'Untitled Service'}</h2>
              <p className="text-muted-foreground text-sm leading-relaxed max-w-2xl whitespace-pre-wrap">
                {serviceInfo.description || 'No description provided.'}
              </p>
            </div>
              ...
              {fields.length === 0 ? (
                ...
                fields.map((field) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`absolute left-2 top-1/2 -translate-y-1/2 p-1.5 text-muted-foreground/40 hover:text-foreground cursor-grab transition-opacity ${isHovered || isSelected ? 'opacity-100' : 'opacity-0'}`}>
                      ...
                      {(isHovered || isSelected) && (
                        <div className="absolute -top-3 right-4 flex items-center bg-card shadow-md border border-border/60 rounded-md overflow-hidden z-20">
                          <button className="p-2 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="Duplicate" onClick={(e) => { e.stopPropagation(); duplicateField(field); }}>
                            <Copy className="w-3.5 h-3.5" />
                          </button>
                          <div className="w-px h-4 bg-border/60" />
                          <button className="p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors" title="Delete" onClick={(e) => { e.stopPropagation(); removeField(field.id); }}>
                      ...
                      <div className="pl-4">
                        ...
                        {field.description && <p className="text-sm text-muted-foreground mb-3">{field.description}</p>}
                        
                        <div className="pointer-events-none w-full max-w-xl">
                          {field.type === 'textarea' ? (
                            <Textarea placeholder={field.placeholder || "Enter your answer..."} className="bg-background/50 border-border/50 min-h-[100px] shadow-sm resize-none" readOnly />
                          ) : field.type === 'dropdown' ? (
                            ...
                            <div className="flex flex-col gap-3">
                              {field.options?.map((opt, i) => (
                                <div key={i} className="flex items-center gap-3">
                                  <div className={`w-4 h-4 border border-border/80 flex items-center justify-center bg-background shadow-sm ${field.type === 'radio' ? 'rounded-full' : 'rounded-[4px]'}`} />
                            ...
                            <Input placeholder={field.placeholder || "Enter your answer..."} className="bg-background/50 border-border/50 shadow-sm" readOnly />
                          )}
                        </div>
              ...
              <div className="p-5 space-y-7">
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input value={selected.label} onChange={(e) => updateField(selected.id, { label: e.target.value })} className="font-medium bg-background border-border/60 shadow-sm focus-visible:ring-1 focus-visible:ring-primary h-9" />
                  </div>
                  ...
                  {['text', 'textarea', 'number', 'email', 'dropdown'].includes(selected.type) && (
                    <div className="space-y-1.5">
                      ...
                      <Input value={selected.placeholder || ''} onChange={(e) => updateField(selected.id, { placeholder: e.target.value })} placeholder="e.g. Enter your name" className="bg-background border-border/60 shadow-sm text-sm h-9" />
                    </div>
                  )}

                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Help Text</Label>
                    <Textarea value={selected.helpText || ''} onChange={(e) => updateField(selected.id, { helpText: e.target.value })} placeholder="Hints to help the user answer..." className="bg-background border-border/60 shadow-sm text-sm min-h-[60px] resize-none" />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg border border-border/60 bg-muted/20">
                    ...
                    <Switch id="req-switch" checked={selected.required} onCheckedChange={(checked) => updateField(selected.id, { required: !!checked })} />
                  </div>
                </div>
                      ...
                      <div className="space-y-2">
                        {selected.options.map((opt, i) => (
                          <div key={i} className="flex items-center gap-2 group/opt bg-background border border-border/50 rounded-md p-1 pl-2 shadow-sm focus-within:ring-1 focus-within:ring-primary/50 focus-within:border-primary/50 transition-all">
                            ...
                            <input value={opt} onChange={(e) => { const newOpts = [...selected.options!]; newOpts[i] = e.target.value; updateField(selected.id, { options: newOpts }); }} className="flex-1 min-w-0 bg-transparent border-0 h-7 text-sm outline-none placeholder:text-muted-foreground/50" placeholder={`Option ${i + 1}`} />
                            <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 text-muted-foreground/50 hover:text-destructive hover:bg-destructive/10 rounded-sm" onClick={() => { const newOpts = selected.options!.filter((_, index) => index !== i); updateField(selected.id, { options: newOpts }); }}>
                              <Trash2 className="w-3.5 h-3.5" />
                            </Button>
                          </div>
                        ))}
                      </div>
                      <Button variant="secondary" size="sm" className="w-full h-8 text-xs font-medium bg-muted/50 hover:bg-muted border border-transparent hover:border-border/50" onClick={() => updateField(selected.id, { options: [...selected.options!, `Option ${selected.options!.length + 1}`] })}>
  ...
  const renderStep3 = () => (
    <div className="flex-1 overflow-y-auto bg-muted/5 flex flex-col items-center py-12 px-6">
      <div className="max-w-3xl w-full space-y-8">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Attach Workflow</h2>
          <p className="text-muted-foreground text-sm mt-1">Select the BPMN workflow that will execute when this service is requested.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { id: 'approval_path', name: 'Standard Approval Path', icon: ShieldCheck, desc: 'Requires manager and IT approval. Automatically notifies users of status changes.' },
            { id: 'auto_provision', name: 'Auto-Provisioning', icon: Server, desc: 'No approvals required. Triggers backend webhook to grant immediate access.' },
            { id: 'custom_wf', name: 'Custom Hardware Workflow', icon: Workflow, desc: 'Your custom drafted workflow designed specifically for hardware requests.' }
          ].map((wf) => {
            const isSelected = selectedWorkflow === wf.id;
            return (
              <Card 
                key={wf.id}
                className={`p-5 cursor-pointer transition-all border-2 ${isSelected ? 'border-primary bg-primary/5 shadow-sm' : 'border-border/50 bg-card hover:border-border hover:shadow-sm'}`}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-medium text-foreground mb-1">{wf.name}</h3>
    ...
    <div className="flex-1 overflow-y-auto bg-muted/5 flex flex-col items-center py-12 px-6">
      <div className="max-w-2xl w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-semibold tracking-tight">Ready to Publish</h2>
          <p className="text-muted-foreground text-sm mt-2 max-w-md mx-auto">Review your service configuration below. Once published, it will be immediately available to your specified audience.</p>
        </div>

        <Card className="divide-y divide-border/50 shadow-sm border-border/50 bg-card">
          <div className="p-6 flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-muted border border-border/60 flex items-center justify-center text-foreground shrink-0">
              {serviceInfo.icon === 'monitor' ? <Monitor className="w-6 h-6" /> : serviceInfo.icon === 'briefcase' ? <Briefcase className="w-6 h-6" /> : serviceInfo.icon === 'box' ? <Box className="w-6 h-6" /> : <Server className="w-6 h-6" />}
            </div>
            <div>
              <h3 className="text-lg font-semibold">{serviceInfo.name}</h3>
              <p className="text-sm text-muted-foreground mt-1 mb-3">{serviceInfo.description}</p>
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="secondary" className="bg-muted text-muted-foreground hover:bg-muted font-normal text-xs">{serviceInfo.category}</Badge>
                <Badge variant="outline" className="text-muted-foreground font-normal text-xs">{serviceInfo.sla} SLA</Badge>
                <Badge variant="outline" className="text-muted-foreground font-normal text-xs capitalize">{serviceInfo.visibility}</Badge>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 grid grid-cols-2 gap-6">
            <div>
              <h4 className="text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-3">Form Configuration</h4>
            ...
            <div>
              <h4 className="text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-3">Linked Workflow</h4>
              <div className="flex items-center gap-2 text-sm text-foreground">
                <Workflow className="w-4 h-4 text-muted-foreground" />
                {selectedWorkflow === 'approval_path' ? 'Standard Approval Path' : selectedWorkflow === 'auto_provision' ? 'Auto-Provisioning' : 'Custom Workflow'}
              </div>
            </div>
          </div>
        </Card>
    ...
    <div className="flex flex-col h-screen bg-[#FDFDFD] dark:bg-background font-sans text-foreground">
      ...
      <header className="flex-none h-14 border-b border-border/40 bg-white/50 dark:bg-background/50 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground shrink-0" onClick={() => currentStep > 1 ? setCurrentStep(c => c - 1) : navigate('/admin/services')}>
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div className="h-4 w-px bg-border/60 mx-1" />
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-muted-foreground cursor-pointer hover:text-foreground transition-colors" onClick={() => navigate('/admin/services')}>Services</span>
            <span className="text-muted-foreground/40 text-sm">/</span>
            <h1 className="text-sm font-semibold tracking-tight">{serviceInfo.name || 'New Service'}</h1>
        ...
        {/* Stepper Center */}
        <div className="hidden md:flex items-center absolute left-1/2 -translate-x-1/2 gap-1.5">
          {[
              ...
              <div key={step} className="flex items-center">
                <div 
                  className={`flex items-center gap-2 px-2 py-1 rounded-md transition-colors ${
                    isCurrent ? 'bg-primary/5 text-primary' : isCompleted ? 'text-foreground' : 'text-muted-foreground/50'
                  }`}
                >
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    isCurrent ? 'bg-primary text-primary-foreground' : isCompleted ? 'bg-primary/20 text-primary' : 'bg-muted border border-border'
                  }`}>
                    {isCompleted ? <Check className="w-3 h-3" /> : step}
                  </div>
                  <span className={`text-xs font-semibold ${isCurrent ? 'text-primary' : isCompleted ? 'text-foreground' : ''}`}>
                    {label}
                  </span>
                </div>
                {index < 3 && <div className={`w-6 h-[1.5px] mx-1 rounded-full ${isCompleted ? 'bg-primary/30' : 'bg-border/50'}`} />}
              </div>
            )
          })}
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center text-xs text-muted-foreground mr-4 animate-in fade-in transition-all">
            {isDraftSaving ? (
              <span className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-full border-2 border-primary border-t-transparent animate-spin" /> Saving...
              </span>
            ) : draftSavedAt ? (
              <span className="flex items-center gap-1.5">
                <Check className="w-3.5 h-3.5" /> Saved {draftSavedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </span>
            ) : null}
          </div>
          {currentStep === 2 && (
            <Button variant="outline" size="sm" className="h-8 gap-1.5 shadow-sm border-border/50 text-xs font-medium mr-2">
              <Eye className="w-3.5 h-3.5 text-muted-foreground" /> Preview Form
            </Button>
          )}
          {currentStep < 4 ? (
            <Button size="sm" className="h-8 gap-1.5 shadow-sm text-xs font-medium" onClick={handleNextStep}>
              Next Step <ArrowRight className="w-3.5 h-3.5 opacity-80" />
            </Button>
          ) : (
            <Button size="sm" className="h-8 gap-1.5 shadow-sm text-xs font-medium" onClick={handlePublish} disabled={isPublishing}>
              {isPublishing ? (
                <>
                  <div className="w-3.5 h-3.5 rounded-full border-2 border-white/40 border-t-white animate-spin mr-1" />
                  Publishing...
                </>
              ) : (
                <>
                  <Save className="w-3.5 h-3.5 opacity-80" /> Publish Service
                </>
              )}
            </Button>
          )}
